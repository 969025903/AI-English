 ...
 *  Copyright © Bean Corporation All rights reserved. [Mr.A.R.B / WJH]
 *  Tip:
 *  *****************************************************************************************
    ...
    public JSONObject parseQueryString  ( String szQueryString, boolean bSafe ){
        ...
        if( szQueryString != null && !szQueryString.isEmpty() ){
            while ( nParseAt < szQueryString.length() ) {
                ...
                try {
                    String szDecodedValueBuf = URLDecoder.decode( hValueBuf.toString(), this.getCharset() );
                ...
                if( hKeyBuf.length() != 0 ){
                    try {
                        hKeyBuf = new StringBuilder( URLDecoder.decode(hKeyBuf.toString(), this.getCharset()) );
                    }
                    catch ( UnsupportedEncodingException e ){
                        e.printStackTrace();
                    }
