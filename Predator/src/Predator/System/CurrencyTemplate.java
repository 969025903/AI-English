...
public class CurrencyTemplate {
    private PredatorWizardSoul mhParentImage;

    private String szPictureMultipleExplorerContent;

    private String szPictureSingleExplorerContent;

    private String szUsingImgUploaderContent;

    private final int nDefaultImageAllowSize = 524288;

    private final int nDefaultFileAllowSize = 2097152;

    private final String szDefaultSrc = "./root/assets/img/userImg.jpg";
    ...
    CurrencyTemplate( PredatorWizardSoul hSoul ) {
        ...
        try{
            ...
            this.szUsingImgUploaderContent        =  this.parent().readFileContentAll( this.getUsingCrispImgUploaderPath()   );
        }
        catch ( IOException e ){
            e.printStackTrace();
        }
    ...
    private String getTemplatePath(){
        return this.parent().getSystemPath()+this.parent().getRealTemplatePath()+this.getPluginsConfig().getString("TemplatePath");
    }

    private Predator parent(){
        return this.mhParentImage.parent();
    }

    private JSONObject getPluginsConfig(){
        return this.parent().getPluginsConfig();
    }


    private String getUsingCrispImgUploaderPath(){
        return this.getTemplatePath()+"UsingImgUploader.html";
    }

    private String getUsingImgUploaderContent(){
        return this.szUsingImgUploaderContent;
    }
    ...
    private String spawnInnerImgUpload(JSONArray hSpawnArray, int nAllowSize, String szSrc){
        StringBuilder szImgUploadStream = new StringBuilder();
        if(hSpawnArray!=null){
            for(int i=0;i<hSpawnArray.length();i++){
                szImgUploadStream.append(" (new CrispImgUploader(\"").append(hSpawnArray.getJSONObject(i).getString("at")).append("\",\"").append(hSpawnArray.getJSONObject(i).getString("name")).append("\",").append(nAllowSize).append(",\"").append(szSrc).append("\")).spawn();");
            }
        }
        return this.getUsingImgUploaderContent()+szImgUploadStream.toString()+"</script>";
    }

    public String usingImgUploader(JSONArray hSpawnArray, String szSrc){
        return this.spawnInnerImgUpload(hSpawnArray,this.nDefaultImageAllowSize,szSrc);
    }

    public String usingImgUploader(JSONArray hSpawnArray, int nAllowSize, String szSrc){
        return this.spawnInnerImgUpload(hSpawnArray,nAllowSize,szSrc);
    }

    public String usingImgUploader(JSONArray hSpawnArray, int nAllowSize){
        return this.spawnInnerImgUpload(hSpawnArray,nAllowSize,this.szDefaultSrc);
    }

    public String usingImgUploader(JSONArray hSpawnArray){
        return this.spawnInnerImgUpload(hSpawnArray,this.nDefaultImageAllowSize,this.szDefaultSrc);
    }
...
//             szFilePathFiled != null ? szFilePathFiled : null
    ...
    public String spawnPredatorPictureMultipleExplorer(String szTableName,String szName,int nSize){
        try {
            String szImageURL = this.parent().mysql().select(
